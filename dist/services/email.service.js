...
exports.sendMeetingCancellationEmail = exports.sendHostNotificationEmail = exports.sendMeetingConfirmationEmail = void 0;
...
const getGmailClient = async (userId) => {
    ...
    const validToken = await (0, integration_service_1.validateGoogleToken)(integration.access_token, integration.refresh_token, integration.expiry_date);
    oauth_config_1.googleOAuth2Client.setCredentials({ access_token: validToken });
    return googleapis_1.google.gmail({ version: 'v1', auth: oauth_config_1.googleOAuth2Client });
};
// Send email using Gmail API
const sendGmailEmail = async (userId, emailContent) => {
    try {
...
const generateMeetingConfirmationEmail = (meeting, event) => {
...
const generateHostNotificationEmail = (meeting, event) => {
    const meetingDate = new Date(meeting.startTime);
    const formattedDate = meetingDate.toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
...
const sendMeetingConfirmationEmail = async (meeting, event) => {
    try {
        const emailContent = {
...
exports.sendMeetingConfirmationEmail = sendMeetingConfirmationEmail;
// Send meeting notification email to host
const sendHostNotificationEmail = async (meeting, event) => {
    try {
        const emailContent = {
            to: event.user.email,
...
exports.sendHostNotificationEmail = sendHostNotificationEmail;
// Send meeting cancellation email
const sendMeetingCancellationEmail = async (meeting, event, cancelledBy) => {
    try {
        const meetingDate = new Date(meeting.startTime);
        const formattedDate = meetingDate.toLocaleDateString('en-US', {
            weekday: 'long',
...
exports.sendMeetingCancellationEmail = sendMeetingCancellationEmail;
